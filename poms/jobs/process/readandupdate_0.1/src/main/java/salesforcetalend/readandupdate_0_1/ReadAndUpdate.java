...
package salesforcetalend.readandupdate_0_1;
 ...
 * @status
 */
public class ReadAndUpdate implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "ReadAndUpdate.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager
			.getLogger(ReadAndUpdate.class);
	...
	private final String jobName = "ReadAndUpdate";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					ReadAndUpdate.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(ReadAndUpdate.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public void tSalesforceConnection_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		mdc("tSalesforceConnection_1", "5laUZ3_");
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				props_tSalesforceConnection_1.userPassword.setValue("securityKey",
						routines.system.PasswordEncryptUtil.decryptPassword(
								"enc:routine.encryption.key.v1:4n2ZjaGTCZKNztu9tG8zAyIGMsTlDuuhK6g3oXM/ifqLV56PRz46KcsYc0JIzo9BbsOuf7k="));
				...
				props_tSalesforceConnection_1.userPassword.setValue("password",
						routines.system.PasswordEncryptUtil.decryptPassword(
								"enc:routine.encryption.key.v1:y2WIGnV91+O3wF/yYvxf0FtywjgonUOaRS7c68JOuZNn4o1D"));
...
// finally of generic
	...
	public static class updatedMapStruct implements routines.system.IPersistableRow<updatedMapStruct> {
		final static byte[] commonByteArrayLock_SALESFORCETALEND_ReadAndUpdate = new byte[0];
		static byte[] commonByteArray_SALESFORCETALEND_ReadAndUpdate = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_SALESFORCETALEND_ReadAndUpdate.length) {
					if (length < 1024 && commonByteArray_SALESFORCETALEND_ReadAndUpdate.length == 0) {
						commonByteArray_SALESFORCETALEND_ReadAndUpdate = new byte[1024];
					} else {
						commonByteArray_SALESFORCETALEND_ReadAndUpdate = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_SALESFORCETALEND_ReadAndUpdate, 0, length);
				strReturn = new String(commonByteArray_SALESFORCETALEND_ReadAndUpdate, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_SALESFORCETALEND_ReadAndUpdate, 0, length);
				strReturn = new String(commonByteArray_SALESFORCETALEND_ReadAndUpdate, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_SALESFORCETALEND_ReadAndUpdate) {
	...
	public static class after_tSalesforceInput_1Struct
			implements routines.system.IPersistableRow<after_tSalesforceInput_1Struct> {
		final static byte[] commonByteArrayLock_SALESFORCETALEND_ReadAndUpdate = new byte[0];
		static byte[] commonByteArray_SALESFORCETALEND_ReadAndUpdate = new byte[0];
	...
	public void tSalesforceInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		mdc("tSalesforceInput_1", "3ZLM9x_");
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				props_tSalesforceOutput_1.setValue("upsertKeyColumn", "ExternalID__c");

				props_tSalesforceOutput_1.upsertRelationTable.setValue("columnName", "ExternalID__c");
				...
				class SchemaSettingTool_tSalesforceOutput_1_1_fisrt {
					...
					String getSchemaValue() {
						...
						a("\"name\":\"Id\",\"type\":\"string\",\"di.table.comment\":\"\",\"AVRO_TECHNICAL_KEY\":\"Id\",\"talend.field.dbColumnName\":\"Id\",\"di.column.talendType\":\"id_String\",\"talend.field.pattern\":\"\",\"talend.field.length\":\"18\",\"di.column.relationshipType\":\"\",\"di.table.label\":\"Id\",\"di.column.relatedEntity\":\"\"}],\"di.table.name\":\"tSalesforceOutput_1\",\"di.table.label\":\"updatedMap\"}",
								s);
...
// finally of generic
	...
	public void tFixedFlowInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// source node:tFixedFlowInput_1 - inputs:(after_tSalesforceInput_1)
				// outputs:(row2,row2) | target node:tAdvancedHash_row2 - inputs:(row2)
				// outputs:()
				// linked node: tMap_1 - inputs:(row1,row2) outputs:(updatedMap)
				...
				for (int i_tFixedFlowInput_1 = 0; i_tFixedFlowInput_1 < 1; i_tFixedFlowInput_1++) {
					...
					if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
							...
							, "row2", "tFixedFlowInput_1", "tFixedFlowInput_1", "tFixedFlowInput", "tAdvancedHash_row2",
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row2", 2, 0,
						"tFixedFlowInput_1", "tFixedFlowInput_1", "tFixedFlowInput", "tAdvancedHash_row2",
	...
	public static void main(String[] args) {
		final ReadAndUpdate ReadAndUpdateClass = new ReadAndUpdate();

		int exitCode = ReadAndUpdateClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'ReadAndUpdate' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		getjobInfo();
		log.info("TalendJob: 'ReadAndUpdate' - Start.");
		...
		org.slf4j.MDC.put("_jobRepositoryId", "_zS-t4PA9Ee-2rNEZYaVnnw");
		org.slf4j.MDC.put("_compiledAtTimestamp", "2025-02-21T10:42:46.468608100Z");
		...
		try {
			...
			java.io.InputStream inContext = ReadAndUpdate.class.getClassLoader()
					.getResourceAsStream("salesforcetalend/readandupdate_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = ReadAndUpdate.class.getClassLoader()
		...
		log.info("TalendJob: 'ReadAndUpdate' - Started.");
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : ReadAndUpdate");
		}
		if (enableLogStash) {
		...
		log.info("TalendJob: 'ReadAndUpdate' - Finished - status: " + status + " returnCode: " + returnCode);
...
/************************************************************************************************
 * 157437 characters generated by Talend Cloud Data Management Platform on the
 * 21. veljače 2025. u 11:42:46 CET
 ************************************************************************************************/